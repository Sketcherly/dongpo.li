...
- 面试评价表管理 面试评价提交
- 测评管理 测评结果、结论接口对接
- 招聘流程管理、招聘流程（招聘阶段-招聘环节流转）
- 测评服务配置管理


#### 2023.12-2024.01
从0开始开发了智菁系统，一个专家调用审批系统。

内部专家管理、外部专家管理、外部专家调用审批管理、专家标签管理、待办管理、鉴权机制。
...
#### 2020.06-2023.10
主要负责公司小程序矩阵的后端服务开发，架构设计，服务维护，线上问题排查等工作。  

1、**用户模块**：参与了用户模块的服务化拆分，将原来单体的用户功能服务化，提供给多个项目使用方便统一管理和维护。  
2、**任务模块**：主导开发了营销推厂类任务墙功能，支持集合多种类型的自有和第三方广告聚合在一个任务列表里使用同样的逻辑完成任务发放奖励。  
3、**签到模块**：主导了签到模块的功能开发以及后续的维护工作，增加用户粘性。  
4、**订阅消息**：主导并维护了订阅消息的功能，通过支付宝提供的订阅消息发送功能，实现增加用户次日留存。  
5、**基础组件**：基于开源组件搭建了调度中心，日志中心，RocketMQ，配置中心等。  


#### 2022.08-2022.10
**投放策略引擎**：基于千川开放平台提供的数据和操作接口，配置一系列策略，根据广告投放的消耗、转化率、ROI等的变化、环比数据，实现千川广告的部分自动化停止、重启、创建、修改日预算等操作。  
**直播中控**：通过Chrome插件油猴脚本定时获取页面的数据和直播间留言，同步上报到服务端，服务端生成简单的报表通过消息或邮件发送非助手。


#### 2022.05-2022.07
果子和肉福利社：同步各个平台(抖店、天猫、小红书、微信小商店、微信视频号小店)的订单数据，根据订单的信息给用户返额外的积分，增加用户粘性和复购。  

1、**订单同步**：同步各个平台的订单，根据用户实付金额给用户返积分。  
2、**兑换商城**：通过周边商品增加吸引力，促进用户积累积分兑换周边，增加用户粘性。  
3、**销售报表**：根据各个平台同步到的订单数据，生成销售额、新老用户、首复购等报表辅助销售。

#### 2020.06-2020.09
财富新思路：理财课程购买，学习的平台。  

1、**课程课时**：添加，上下架课程，课时的语音、文字版本。  
2、**交易模块**：用户购买课程下单、支付、查看购买记录、支付成功根据实付金额返积分等。  
3、**听课记录**：记录用户听课开始结束时间，根据用户听课时长给用户奖励积分。  
4、**兑换商城**：用户使用积累的积分兑换现金红包，异步发放到用户支付宝账户。  
...
- 商务管理包括:维护量管理和费用管理，维护量管理包括项目维护两确认、项目维护量统计和机房维护量统计，费用管理包括维护费确认、维护费统计、单价管理、单价修订历史、计件管理、计件修订历史等模块。
